...
def main():
    ...
    with st.sidebar:
        ...
        # Don't show default assistant ID, use a placeholder instead
        new_assistant_id = st.text_input(
            "Assistant ID",
            placeholder="Enter new Assistant ID",
            value="")

        if st.button("Update Assistant ID") and new_assistant_id:
            st.session_state.current_assistant_id = new_assistant_id
            create_new_thread()
            st.success("Assistant ID updated! New chat thread created.")
            st.rerun()

        # Remove the caption that shows the current Assistant ID
        # st.caption(f"Current Assistant ID: {st.session_state.current_assistant_id}")
